...
- [x] **`POST /v2/templates/realign`** — re-align an existing hmmsearch-style A3M against the query under OTalign / Ankh-Large / glocal. Sync endpoint, bearer-gated like `/v2/embed`. Output rows are exactly `query_len` chars from `[A-Z-]` (no lowercase A3M insertions; template residues OTalign couldn't place at a query column are dropped). Headers preserve domain id + every tail token, get a re-intervalled `/start-end` matching the placed span, and a `Score=N.NNN` token inserted right after the range. Empirical: 593-record CASP-style fixture re-aligns in ~12 s on one Ada GPU; 6.4× embedding-call dedup payoff. See [PLAN_TEMPLATES_REALIGN.md](./PLAN_TEMPLATES_REALIGN.md) for design rationale (mode default, score-placement, no-lowercase rule). Operator walkthrough at [docs/templates-realign.md](./docs/templates-realign.md).
