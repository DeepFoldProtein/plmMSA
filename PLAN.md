...
### M9 — templates re-alignment
- [x] **`POST /v2/templates/realign`** — re-align an existing hmmsearch-style A3M against the query under OTalign / Ankh-Large / glocal. Sync endpoint, bearer-gated like `/v2/embed`. Output mirrors the hmmsearch input shape (no synthetic query record at top); rows are exactly `query_len` chars from `[A-Z-]` (no lowercase A3M insertions; template residues OTalign couldn't place at a query column are dropped). Headers preserve domain id + every tail token, get a re-intervalled `/start-end` matching the placed span, and a `score:N.NNN` token slotted at the end of the technical-tokens section (right before the double-space separator that prefixes the free-text description). Empirical: 593-record CASP-style fixture re-aligns in ~12 s on one Ada GPU; 6.4× embedding-call dedup payoff. See [PLAN_TEMPLATES_REALIGN.md](./PLAN_TEMPLATES_REALIGN.md) for design rationale (mode default, score-placement, no-lowercase rule). Operator walkthrough at [docs/templates-realign.md](./docs/templates-realign.md).
- [x] **glocal as the default mode** — head-to-head comparison on the fixture sample showed q2t loses ~25 pp on hmmsearch agreement to its forced `query_end == Lq` artifact. q2t / local / global / t2q all stay callable via the `mode` field for callers who need a different end-gap policy.
- [x] **`bin/realign_fixture.py`** — one-shot operator script: runs the orchestrator end-to-end on the bundled fixture and writes `tmp/exostosin_realigned.a3m` + a stats JSON. Reproducible sample run for the docs.
